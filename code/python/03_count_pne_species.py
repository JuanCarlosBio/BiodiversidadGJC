...
plantae_data = (
    pd.DataFrame(plantae_plus_pne[["codigo", "category", "id_biota"]])
        .groupby(["codigo", "category", "id_biota"])
        .size().reset_index(name = "n")
        .groupby(["codigo", "category"])
        .size()
        .reset_index(name = "n")
    )

plantae_data.to_csv("data/species/pne_species_count.csv", index=False)