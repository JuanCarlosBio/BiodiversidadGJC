...
geometry_plantae = [Point(xy) for xy in zip(df_plantae['longitude'], df_plantae['latitude'])]
...
plantae_data = pd.DataFrame(plantae_plus_pne[["codigo", "category", "id_biota"]]).groupby(["codigo", "category", "id_biota"]).size().reset_index(name = "n").groupby(["codigo", "category"]).size().reset_index(name = "n") 
